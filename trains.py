...
# 批量将16x32的Tensor转换为16个32位的整数
def tensor_to_ints(tensor):
    results = []
    for row in tensor:
        result = 0
        for bit in row:
            result = (result << 1) | int(bit.item())
...
def train_model(model, trainloader, testloader,label_hash_codes, epochs=epochs, hash_bits = 64):
    ...
    handler1 = logging.FileHandler(f'./logs/{trainloader.dataset.noise_type}_{trainloader.dataset.noise_rate}_time.log')
    handler1.setFormatter(logging.Formatter('%(asctime)s:%(levelname)s:%(message)s'))
    logger1.addHandler(handler1)

    #logging.basicConfig(filename=f'./logs/{trainloader.dataset.noise_type}_{trainloader.dataset.noise_rate}_time.log', level=logging.INFO,
    ...
    """
    label_hash_codes=label_hash_codes.add(1).div(2).to(device)
    label_hash_codes = label_hash_codes.to(torch.uint8)  
    label_hash_codes = tensor_to_ints(label_hash_codes)
    """
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if epoch > 0:
            epoch_duration = epoch_times[-1] - epoch_times[-2]  # 计算这个epoch和上一个epoch之间的时间差
            logger1.info(f'Epoch:{epoch} time:{epoch_duration:.2f}')
        ...
        for iter, (inputs, labels,is_noise) in enumerate(trainloader):
            ...
            simcodes = torch.sign(outputs).add(1).div(2).to(torch.uint8)
            ...
            predicted_label = get_predicted_label(outputs,label_hash_codes,device)
            ...
            loss.backward()
            optimizer.step()
        peak_mem = torch.cuda.max_memory_allocated(device=device) / (1024 ** 3)  # 转换为GB
        logger1.info(f'Epoch {epoch} - Peak Memory Usage: {peak_mem:.2f} GB')
        print(f'Epoch {epoch} - Peak Memory Usage: {peak_mem:.2f} GB')
